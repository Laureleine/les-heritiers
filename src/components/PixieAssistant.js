...
export default function PixieAssistant({ character, step, session, onSleep, fairyData }) {
    ...
    // ✨ FIX 1 : On stabilise la fonction de vol
    ...
    // ✨ FIX 2 : On stabilise la fonction de fermeture (qui dépend du vol)
    ...
    // ✨ FIX 3 : Le useEffect possède maintenant des dépendances exhaustives et stables !
    useEffect(() => {
        if (isTalking) {
        ...
        const verifierRecompensesForge = async () => {
            try {
                ...
                if (!error && data) {
                    ...
                    // On valide immédiatement en base pour que Pixie ne répète pas
                    await supabase
                        .from('registre_forge')
                        .update({ is_notified_pixie: true })
                        .eq('id', data.id);

                    // On déclenche l'animation de Pixie !
            ...
            >
                {isTalking && (
                    <div className={`absolute bottom-full mb-4 -left-32 w-64 p-3 rounded-2xl shadow-xl border-2 pointer-events-auto animate-fade-in-up ${getBubbleColors(message.mood)}`}>
                        ...
                        <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 border-b-2 border-r-2 transform rotate-45 ${getBubbleColors(message.mood).split(' ')} ${getBubbleColors(message.mood).split(' ')}`}></div>
                    </div>
                )}
                <button
