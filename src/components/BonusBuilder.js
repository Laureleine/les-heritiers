...
export default function BonusBuilder({
  ...
  const generateTechJson = (currentBlocks) => {
    ...
    currentBlocks.forEach(b => {
      ...
      // ✨ NOUVEAU : Écriture des Ajustements de Prix
  ...
  const addBlock = (type) => {
    ...
    else if (type === 'comp') newBlock.key = usefulSkills || 'Mêlée';
    else if (type === 'price_modifier') newBlock.val = 0; // Prix par défaut à 0
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-stone-200 rounded-xl shadow-xl z-20 overflow-hidden animate-fade-in max-h-96 overflow-y-auto">
                {BLOCK_TYPES.map(bt => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-sm font-bold flex items-center gap-3 hover:bg-stone-50 transition-colors border-b border-stone-100 last:border-0 ${bt.color.replace('bg-', 'text-').split(' ')[1]}`}
                  >
                    <div className={`p-1.5 rounded-md ${bt.color.split(' ')} ${bt.color.split(' ')[1]}`}>
                      {bt.icon}
                    </div>
                    {bt.label}
                  </button>
      ...
      <div className="space-y-3">
        ...
        {blocks.map(block => (
          <div key={block.id} className="flex items-center gap-3 bg-white p-2 rounded-lg shadow-sm border border-stone-200 animate-fade-in group">
            ...
            {/* ✨ NOUVEAU : Lego Ajustement de Prix (Rose) */}
