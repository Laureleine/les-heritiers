      ...
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl border border-amber-100 overflow-hidden">
        <header className="bg-amber-900 text-white p-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              ...
              <p className="text-amber-200 italic mt-1 text-sm">
                Belle Époque Paris v{APP_VERSION}
              </p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setView('list')}
                className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-lg hover:bg-white/20 transition-all font-serif text-sm border border-white/20"
              >
                <List size={16} />
                Mes personnages
              </button>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-700 font-serif text-sm"
              >
                <FileText size={16} />
                PDF
              </button>
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-6 py-2 bg-green-600 rounded-lg hover:bg-green-700 shadow-lg font-serif text-sm"
              >
                <Save size={16} />
                Sauvegarder
              </button>
        ...
        <div className="mt-8 flex items-center justify-between gap-2 max-w-3xl mx-auto">
          {steps.map(s => (
            <React.Fragment key={s}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-serif text-xs ${
                step === s 
              ...
              {s < 8 && (
                <div className={`flex-1 h-0.5 ${
                  step > s ? 'bg-amber-500' : 'bg-amber-950'
                }`} />
              )}
            </React.Fragment>
          ))}
        ...
        <main className="p-8 md:p-12">
          {step === 1 && (
            <Step1
              character={character}
              onNomChange={(n) => setCharacter({ ...character, nom: n })}
              onSexeChange={(s) => setCharacter({ ...character, sexe: s })}
              onTypeFeeChange={(t) => {
                const ancienne = getFairyAge(t, 'traditionnelle', gameData.fairyData);
                setCharacter({ ...character, typeFee: t, anciennete: ancienne });
              }}
          ...
          {step === 2 && (
            <StepCaracteristiques
              character={character}
              onCaracteristiquesChange={(c) => setCharacter({ ...character, caracteristiques: c })}
          ...
          {step === 4 && (
            <StepCompetencesLibres
              character={character}
              onCompetencesLibresChange={(cl) => setCharacter({ ...character, competencesLibres: cl })}
              fairyData={gameData.fairyData}
          ...
          {step === 5 && (
            <StepCompetencesFutiles
              character={character}
              onCompetencesFutilesChange={(cf) => setCharacter({ ...character, competencesFutiles: cf })}
              fairyData={gameData.fairyData}
            />
          )}
          ...
          {step === 6 && (
            <Step2
              character={character}
              onCapaciteChoisie={(c) => setCharacter({ ...character, capaciteChoisie: c })}
              fairyData={gameData.fairyData}
            />
          )}
          ...
          {step === 7 && (
            <Step3
              character={character}
              onPouvoirToggle={(p) => {
                if (character.pouvoirs.includes(p)) {
                  setCharacter({
                    ...character,
                    pouvoirs: character.pouvoirs.filter(x => x !== p)
                  });
                } else {
                  setCharacter({
                    ...character,
                    pouvoirs: [...character.pouvoirs, p]
                  });
                }
              }}
              fairyData={gameData.fairyData}
        ...
        <div className="mt-12 pt-8 border-t border-amber-100 flex justify-between">
          <button
            ...
            disabled={step === 1}
            className="flex items-center gap-2 px-6 py-3 bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200 disabled:opacity-50 font-serif"
          >
            <ChevronLeft size={20} />
            Précédent
          </button>
          ...
          <button
            ...
            disabled={
              ...
              step === 6 && !character.capaciteChoisie ||
          ...
          >
            {step === 8 ? 'Terminer et Sauver' : 'Suivant'}
