...
export default function DiceRoller({ use3DDice = false, diceTheme = 'laiton' }) {
    ...
    // ✨ FIX : Suppression de `rollingMode` qui ne servait à rien
    ...
    useEffect(() => {
        // ✨ FIX : Capture locale de la ref pour satisfaire les règles strictes de React (exhaustive-deps)
        const timer = rollTimerRef.current;
