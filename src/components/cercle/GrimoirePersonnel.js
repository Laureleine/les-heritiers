...
export default function GrimoirePersonnel({ characterId, cercleId, playerId }) {
  ...
  const handleShareNote = (noteId) => {
    // Logique future de bascule RLS (Scénario A)
    showInAppNotification("La missive a été partagée avec le Cercle.", "success");
  };
                ...
                return (
                  <div key={contact.id} className="p-4 border border-stone-200 rounded-xl bg-white shadow-sm flex flex-col gap-3 relative overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider shrink-0 ${
                        data.statut_relation === 'Allié' ? 'bg-green-100 text-green-800 border border-green-200' :
