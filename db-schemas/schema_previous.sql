...
\restrict fbFDrM24hpav8gVAcbPvEckhsxhvE4XjYBUdEIxCdqHhnlA0brzwTjBRZOB31PT
...
BEGIN
  SELECT json_build_object(
    ...
    'new_profiles', (
      SELECT json_agg(json_build_object('created_at', created_at))
      FROM profiles WHERE created_at >= thirty_days_ago
    ),
    'character_activity', (
      SELECT json_agg(json_build_object('created_at', created_at, 'updated_at', updated_at))
      FROM characters WHERE created_at >= thirty_days_ago OR updated_at >= thirty_days_ago
    ),
    'encyclopedia_requests', (
      SELECT json_agg(json_build_object('created_at', created_at))
      FROM data_change_requests WHERE created_at >= thirty_days_ago
    )
  ) INTO result;
  RETURN result;
END;
...
CREATE TABLE public.menu_plats (
    ...
    CONSTRAINT menu_plats_statut_check CHECK ((statut = ANY (ARRAY['en_attente'::text, 'approuve'::text, 'refuse'::text])))
);
...
--
-- Name: cercle_membres cercle_membres_cercle_id_user_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cercle_membres
    ADD CONSTRAINT cercle_membres_cercle_id_user_id_key UNIQUE (cercle_id, user_id);
...
--
-- Name: idx_menu_plats_auteur; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_menu_plats_auteur ON public.menu_plats USING btree (auteur_id);
...
--
-- Name: idx_menu_plats_statut; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_menu_plats_statut ON public.menu_plats USING btree (statut);
...
ALTER TABLE ONLY public.menu_plats
    ADD CONSTRAINT menu_plats_auteur_id_fkey FOREIGN KEY (auteur_id) REFERENCES auth.users(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY public.menu_plats
    ADD CONSTRAINT menu_plats_validateur_id_fkey FOREIGN KEY (validateur_id) REFERENCES auth.users(id) ON DELETE SET NULL;
...
CREATE POLICY menu_plats_insert ON public.menu_plats FOR INSERT TO authenticated WITH CHECK (((auteur_id = auth.uid()) AND ((statut = 'en_attente'::text) OR (EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['super_admin'::text, 'gardien'::text]))))))));
...
CREATE POLICY menu_plats_select ON public.menu_plats FOR SELECT TO authenticated USING (((statut = 'approuve'::text) OR (auteur_id = auth.uid())));
...
\unrestrict fbFDrM24hpav8gVAcbPvEckhsxhvE4XjYBUdEIxCdqHhnlA0brzwTjBRZOB31PT
