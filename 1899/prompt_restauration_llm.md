...
- **Entrée :** Un fichier ou un bloc de données JSON contenant les articles découpés pour une date cible. Repère l'en-tête du fichier (en commentaires) pour connaître la date et la passe précédente (ex: `// Date: 1899-11-27` et `// Restauration Pass: 1`). Si les commentaires ne sont pas présents ou si la date ne correspond pas à la date cible, considère qu'il s'agit de la passe `0` (texte brut).
- **Sortie :** Le même format de structure de données mais :
  ...
  3. **Obligatoirement précédé de l'en-tête de commentaire indiquant le verrou de date et le nouveau compteur de passe** incrémenté de 1 (ex : `// Date: 1899-11-27` puis en dessous `// Restauration Pass: 2`).
